...
function App() {
  ...
  useEffect(() => {
    localStorage.setItem("dailyHabits", JSON.stringify(tasks));
  }, [tasks]);
  ...
  const addTask = () => {
    if (task.trim() === "" || start === "" || due === "") {
      alert("Enter habit, start time and due time");
      return;
    }
  ...
  const getRemaining = (item) => {
    ...
    if (difference < 0) return "Time exceeded";
  ...
  return (
    <div className="app">
      <header>
        <h2>Daily Habit & Activity Tracker</h2>
        <p>📅 Daily Planner</p>
      </header>
      ...
      <main>
        <div className="add-box">
          ...
          <button onClick={addTask}>+ Add Habit</button>
        </div>

        <h3>Today's Activities</h3>

        {activeTasks.length === 0 && <p className="empty">No active habits</p>}
            ...
            <div className={`task ${status.toLowerCase()}`} key={index}>
              <div className="task-top">
                ...
                <div className="task-info">
                  ...
                  <small>Target: {new Date(item.due).toLocaleString()}</small>
                </div>
